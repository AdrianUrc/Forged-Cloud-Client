...
fn load_tls_client_config() -> ClientConfig {
    let mut ca_reader = BufReader::new(File::open("ca.pem").expect("cannot open ca.pem"));
    let mut root_store = RootCertStore::empty();
    root_store
        .add_parsable_certificates(&certs(&mut ca_reader).expect("failed to read CA certificate"));

    let mut cert_reader =
        BufReader::new(File::open("client-cert.pem").expect("cannot open client-cert.pem"));
    let client_cert_chain: Vec<Certificate> = certs(&mut cert_reader)
        ...
        .collect();
    let mut key_reader =
        BufReader::new(File::open("client-key.pem").expect("cannot open client_key.pem"));
